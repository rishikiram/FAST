...
# ------------------- INPUTS ------------------------------------------------
fork_dir_raw = '/home/rootrish/FORK/raw/'
fork_dir_detrend = '/home/rootrish/FORK/detrend/'
...
#    print(file_str)
   ...
   # Read file
   st = read(os.path.join(fork_dir_raw, file_str), format=format_str)
...
#	 st[itr].decimate(dec_factor)
   ...
   if (format_str == 'MSEED'):
      for itr in range(len(st)):
        ...
        #  print(st[itr].data)
   st.write(os.path.join(fork_dir_detrend, file_str), format=format_str)
